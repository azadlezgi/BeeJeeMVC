    ...
    public function get() {
        ...
        if (!is_null($links)) {
            if ($this->current_page > 1) {
                $links = $this->generateHtml(1, '<i class="fa fa-long-arrow-left" aria-hidden="true"></i>').$links;
            }
            if ($this->current_page < $this->amount) {
                $links .= $this->generateHtml($this->amount, '<i class="fa fa-long-arrow-right" aria-hidden="true"></i>');
            }
        }
        $html .= $links.' </ul></nav>';
        return $html;
